...
func TestNewExtractRequestPool(t *testing.T) {
	...
	for i := 0; i < 20; i += 2 {
		req := pool.Get(i)
		cstruct, freeCStruct := allocSSPluginExtractField(5, ParamTypeUint64, "test.field", "arg", false)
...
func TestExtractRequestSetValue(t *testing.T) {
	...
	pool := NewExtractRequestPool()
	u64Ptr, freeU64Ptr := allocSSPluginExtractField(1, ParamTypeUint64, "test.u64", "", false)
	u64ListPtr, freeU64ListPtr := allocSSPluginExtractField(1, ParamTypeUint64, "test.u64", "", true)
	strPtr, freeStrPtr := allocSSPluginExtractField(2, ParamTypeCharBuf, "test.str", "", false)
	strListPtr, freeStrListPtr := allocSSPluginExtractField(2, ParamTypeCharBuf, "test.str", "", true)
	u64Req := pool.Get(0)
